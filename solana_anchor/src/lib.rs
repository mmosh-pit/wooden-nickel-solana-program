...
pub mod solana_anchor {
    ...
    pub fn mint(
        ...
        let substring: String =_data.name.clone().replace(std::str::from_utf8(&vec![0u8]).unwrap(), "").to_owned() + &"'s Wooden Nickel".to_owned();
        ...
        pool.creator = *ctx.accounts.nft_mint.key;
        Ok(())
    }
}
...
pub const METADATA_EXTENDED_SIZE : usize = 32;
#[account]
...
pub const CONFIG_SIZE : usize = 32 + 4 + CONFIG_DATA_SIZE; 
#[account]
#[derive(Default)]
pub struct Config{
